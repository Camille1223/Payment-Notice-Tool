...
def docx_to_pdf_libreoffice(docx_path: str, output_dir: str) -> str | None:
    """Convert docx to PDF using LibreOffice. Returns pdf path or None on failure."""
    try:
        ...
        app.logger.warning("LibreOffice error: %s", result.stderr)
        return None
    except Exception as e:
        app.logger.warning("LibreOffice exception: %s", e)
        return None
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
            for i, row in enumerate(rows, 1):
                ...
                pdf_path = docx_to_pdf_libreoffice(docx_out, tmpdir)
                if pdf_path:
                    zf.write(pdf_path, f"PDF/{unique_name}.pdf")
                    pdf_count += 1
